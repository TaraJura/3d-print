...
Stav k 19. 9. 2026: **první stolní rozběh skutečného motoru přes telefon, Arduino UNO R4 WiFi a ST L293D uživatel potvrdil.** Po stisku tlačítka motor běžel přibližně sekundu. Po vyjmutí jedné baterie už neběžel; přerušil se celý sériový bateriový obvod. [Souhrnný záznam dnešního sestavování a výsledků](prvni-stolni-test.md) je výchozí bod pro pokračování. [Zapojení](zapojeni-l293d.md) a [firmware](firmware/prvni-motor/README.md) jsou uložené zde. Jízda celého autíčka a provoz pod zátěží zůstávají neověřené.

Arduino bylo při zkoušce na USB powerbance; motorová větev používala držák čtyř AA se **směsí Ni-MH 1,2 V a běžných článků**, bez změřeného celkového napětí. Kondenzátory nebyly osazené, vodiče motoru jsou dočasně zahnuté kolem kontaktů bez pájení. Tyto podmínky popisují provedený pokus, nejsou hotovým řešením pro provoz autíčka. Poslední potvrzený stav má jednu AA vyjmutou; následné odpojení USB není doložené.

**Pozdější stav:** USB diagnostika po opětovném výpadku prokázala trvalé zablokování obsluhy po výsledku Wi-Fi 255. Agent následně nahrál opravu, která po návratu platného stavu obnoví web a odmítá staré či opožděné motorové požadavky; zapojení a sekundový pulz zůstaly stejné. Překlad, regresní testy a upload uspěly. Uživatelské potvrzení funkční sestavy před tímto uploadem patří ještě diagnostické verzi. Telefon a motor s aktuální opravou čekají na opakovanou zkoušku; důvod samotného chybového výsledku modemu není prokázaný. [Diagnostika webu a evidence opravy](diagnostika-webu.md).
