...
(async()=>{
  ...
  for(const first of ['drive','left'])await test('real multitouch '+first+' first, independent release and safe stop',{},async f=>{
    ...
    await until(()=>first==='drive'?f.state.motor===128:f.state.steeringUs===1200,'first control');
    ...
    await until(()=>f.state.motor===128&&f.state.steeringUs===1200,'motor+left from both fingers');
    ...
    if(first==='left'){
      ...
      await until(()=>f.state.motor===0&&f.state.steeringUs===1200,'motor released, steering stays');
    }else{
  ...
  await test('servo-only keyboard, explicit center, combined arrows and focus loss',{},async f=>{
    await f.page.keyboard.down('ArrowRight');await until(()=>f.state.steeringUs===1800,'right only');assert.equal(f.state.motor,0);
    ...
    await f.page.keyboard.up('ArrowRight');await f.page.keyboard.down('ArrowLeft');await until(()=>f.state.steeringUs===1200,'left');
  ...
  await test('dropped STOP with both actuators returns motorOFF/neutral without new input',{},async f=>{
    ...
    await until(()=>f.state.motor===128&&f.state.steeringUs===1200,'combined input');
