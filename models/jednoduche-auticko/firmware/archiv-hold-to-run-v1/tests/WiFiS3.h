...
inline void pinMode(int pin,int){if(pin!=5&&pin!=7&&pin!=8&&pin!=13)throw std::runtime_error("unexpected output pin");}
...
struct ClientState {std::string input,output;size_t at=0;bool alive=true;uint32_t next=0,gap=0;};
struct WiFiClient {
  ...
  void print(const char *s){io();blockOnce(blocked.print);state->output+=s;}
  void print(size_t n){io();blockOnce(blocked.print);state->output+=std::to_string(n);}
  size_t write(const uint8_t *data,size_t size){io();blockOnce(blocked.write);state->output.append(reinterpret_cast<const char *>(data),size);return size;}
