...
parser=argparse.ArgumentParser();parser.add_argument('input_zip');parser.add_argument('output_dir');parser.add_argument('--attempts',type=int,default=100)
...
def save_layout(plates):
 ...
 content={'units':'mm','transform_convention':'Rotate original STL about +Z counterclockwise by angle_deg (right-handed), then translate by dx_mm,dy_mm,dz_mm; scale=1. Z unchanged. Plate origin is front-left0,0.','source_zip':str(Path(args.input_zip).resolve()),'source_zip_sha256':hashlib.sha256(Path(args.input_zip).read_bytes()).hexdigest(),'plate_mm':[260,260],'clearance_policy':{'minimum_hull_gap_mm':11.0,'minimum_edge_mm':6.0,'process_assumption':'brim_width=5mm, brim_object_gap=0.1mm, no skirt; supports disabled in verified default profile. Local support toolpaths have not been sliced or verified and must fit inside the reserved envelope.'},'count':len(result),'items':result,'proof':proof,'heuristic':'convex hulls over all Z, no nesting in holes; greedy no-fit polygon search, rotations0/90/180/270; no claim global optimum'}
 (out/'layout.json').write_text(json.dumps(content,indent=2)+'\n')
 return content

# Reproducible sequence explores greedy orders, always preserving actual part copies.
rng=random.Random(20260919);start=time.time();best=None;bestmissing=99
frame='ram-rizeni__01.stl';platform='plosina-170x60__01.stl';target=[n for n in N if n not in [frame,platform]]
# Start with sensible largest-first orders.
for attempt in range(args.attempts):
 mode=attempt%4
 if attempt<4:order=sorted(target,key=lambda n:raw[n]['hull'].area,reverse=True)
 else:
  order=sorted(target,key=lambda n:raw[n]['hull'].area*rng.uniform(.6,1.7),reverse=True)
 placed,missing=pack(order,mode)
 if len(missing)<bestmissing:
  best=(placed,missing);bestmissing=len(missing)
  plate2,more=pack([frame,platform]+missing,0)
  if not more:
   save_layout([placed,plate2]);print('BEST',len(placed),len(plate2),'attempt',attempt,'seconds',round(time.time()-start,1),flush=True)
 if not missing:break
 if attempt%5==0:print('progress',attempt,'best',31-bestmissing,'cache',len(cache),'sec',round(time.time()-start,1),flush=True)
print('Done best first',31-bestmissing,'sec',round(time.time()-start,1),flush=True)
