...
p=argparse.ArgumentParser();p.add_argument('layout_json');p.add_argument('output_dir');p.add_argument('--input-zip');a=p.parse_args()
...
with zipfile.ZipFile(a.input_zip or d['source_zip']) as z:
 for plate in sorted({i['plate'] for i in d['items']}):
  ...
  for j,l in enumerate(labels):fig.text(.67,.875-j*.022,l,fontsize=8.5,fontfamily='monospace',color='#26374e')
  fig.text(.055,.955,f'Autíčko se zatáčením · podložka {plate} · {len(items)} kusů',fontsize=19,fontweight='bold',color='#1b2c43')
  ...
  path=out/f'podlozka-{plate}.png';fig.savefig(path,dpi=160);plt.close(fig);print(path)
