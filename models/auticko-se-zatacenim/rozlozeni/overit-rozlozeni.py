...
with zipfile.ZipFile(a.input_zip) as z:
 expected=sorted(n for n in z.namelist() if n.endswith('.stl'));actual=sorted(i['name'] for i in d['items']);assert expected==actual;assert len(expected)==33;assert len(set(actual))==33
 for i in d['items']:
...
for plate,parts in sorted(groups.items()):
 ...
 minimum=min(x['clearance_mm'] for x in distances);edge=min(min(h.bounds[0],h.bounds[1],260-h.bounds[2],260-h.bounds[3]) for _,h in parts)
...
proof['count']=33;proof['all_passed']=True;proof['method']='All original STL vertices, all-height convex XY hulls, pairwise Euclidean distances, no hole nesting; rigid Z rotations and XY translations only. Brim assumption width5mm + objectgap0.1mm. Does not prove support toolpath envelope or sliced layers.'
