...
def write_plate(out,items,source):
    ...
    ET.SubElement(model,tag('metadata'),{'name':'Description'}).text='V3 Kobra X 260x260: pouze geometrie a rozlozeni, bez tiskovych nastaveni.'
...
def main():
    ...
    args=ap.parse_args();d=json.loads(args.layout.read_text());items=d['items']
    args.output.mkdir(parents=True,exist_ok=True)
    with zipfile.ZipFile(args.zip) as z:
        assert sorted(p['name'] for p in items)==sorted(n for n in z.namelist() if n.lower().endswith('.stl'))
        files=[write_plate(args.output/f'auticko-v3-podlozka-{b:02d}.3mf',[p for p in items if p['plate']==b],z) for b in sorted({p['plate'] for p in items})]
