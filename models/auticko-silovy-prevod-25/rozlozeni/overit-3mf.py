...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('zip',type=Path);ap.add_argument('layout',type=Path);ap.add_argument('directory',type=Path);ap.add_argument('--prefix',default='auticko-25');ap.add_argument('--expected-wheels',type=int,default=4);ap.add_argument('--wheel-part',action='append',default=[]);args=ap.parse_args()
 layout=json.loads(args.layout.read_text());assert not any('rotation_matrix' in i for i in layout['items']), 'Historical XY checker; use stojate-cepy-2026-09-20/pripravit.py and overit.py';assert layout['source_zip_sha256']==hashlib.sha256(args.zip.read_bytes()).hexdigest();items=layout['items'];assert all(x['scale']==1 and x['dz_mm']==0 for x in items);positions={Path(p['name']).stem:p for p in items};seen=[];plates=[]
