...
def write_plate(out,items,source):
    ...
    ET.SubElement(model,tag('metadata'),{'name':'Description'}).text='Auticko 25:1 Kobra X 260x260: pouze geometrie a rozlozeni, bez tiskovych nastaveni.'
    ...
    for i,p in enumerate(items,1):
        ...
        for x,y,z in vertices:
            if 'rotation_matrix' in p:
                q=[sum(r[k]*v for k,v in enumerate((x,y,z)))+p['translation_mm'][j] for j,r in enumerate(p['rotation_matrix'])]
            else:
                q=[x*c-y*s+p['dx_mm'],x*s+y*c+p['dy_mm'],z]
            ET.SubElement(vs,tag('vertex'),dict(zip(['x','y','z'],[f'{v:.9f}' for v in q])))
...
def main():
    ...
    ap.add_argument('--prefix',default='auticko-25');args=ap.parse_args();d=json.loads(args.layout.read_text());items=d['items']
    if args.output.resolve()==Path(__file__).resolve().parent:
        raise SystemExit('Canonical layout is protected: plate 1 is user-saved. Use a new output directory, and stojate-cepy-2026-09-20/pripravit.py for the current upright plate 3.')
