...
def main():
    ...
    if args.calibration:
        ...
        # Count/copy audit is independent of unverified mechanical prototype acceptance.
        assert all(p['valid'] is True and p['solid_count'] == 1 and p['closed_mesh'] is True for p in parts.values())
    ...
    with zipfile.ZipFile(args.zip) as archive:
        ...
        for name in entries:
            ...
            recorded_sha = parts[canonical].get('sha256', parts[canonical].get('stl_sha256'))
            if recorded_sha: assert sha(original) == recorded_sha, ('Source report versus canonical STL hash', canonical)
