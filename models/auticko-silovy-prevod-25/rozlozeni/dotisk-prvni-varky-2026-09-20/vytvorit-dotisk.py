...
PLAN = [(8,1,RY_MINUS_90,(130,130)), (11,2,RY_MINUS_90,(130,130)),
...
TITLES = {1:'Zadní osa · nastojato s lokálními podporami', 2:'Pravá těhlice · otočená s podporami',
...
def write3mf(path, parts):
    ...
    if path.exists():
        with ZipFile(path) as before, ZipFile(io.BytesIO(output.getvalue())) as after:
            if before.namelist()==after.namelist() and all(before.read(n)==after.read(n) for n in after.namelist()):
                return  # Preserve exact archive hash when geometry did not change.
...
def main():
    ...
    report={'status':'pass','units':'mm','counts':[1,1,5],'total_repair_copies':7,'source_zip_sha256':sha((MODEL/'tiskovy-balicek.zip').read_bytes()),
            'orientation_only':True,'original_cad_stl_unchanged':True,'print_order':'by layer',
