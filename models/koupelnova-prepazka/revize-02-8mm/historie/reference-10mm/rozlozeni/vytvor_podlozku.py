...
# Piece counts, 12 mm overlaps and individual lengths remain design choices.
PARTS = [
    ("dil-1.stl", (196.0, 20.0, 10.0), (32.0, 70.0, 0.0)),
    ("dil-2.stl", (202.0, 20.0, 10.0), (29.0, 110.0, 0.0)),
    ("dil-3.stl", (196.0, 20.0, 10.0), (32.0, 150.0, 0.0)),
]
...
def main():
    ...
    parser.add_argument('--height-mm', type=float, default=10.0, help='Nominal CAD height for geometry validation; does not scale STL')
    args = parser.parse_args()
    require(args.height_mm > 0, 'Expected CAD height must be positive')
    parts = [(name, (size[0], size[1], args.height_mm), translation) for name, size, translation in PARTS]
    ...
    ET.SubElement(model, "metadata", {"name": "Title"}).text = "Přepážka B — pracovní geometrie, 3 díly"
    ...
    require(len(objects) == len(items) == len(parts) == 3, "Wrong object or instance count")
    ...
    result = {
        ...
        "physical_parts": 3, "object_instances": 3, "contains_process_or_filament": False, "contains_gcode": False,
    ...
    print(json.dumps({"3mf": str(OUTPUT), "audit": str(AUDIT), "parts": 3,
