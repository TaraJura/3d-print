...
OUTPUT = HERE / "prepazka-B-pracovni-geometrie.3mf"
...
# Maximal external width corrected by Jiří to 20 mm on 2026-09-22.
# Piece counts, joints and individual lengths remain design choices.
PARTS = [
    ("dil-1.stl", (193.0, 20.0, 8.0), (33.5, 70.0, 0.0)),
    ("dil-2.stl", (200.0, 20.0, 8.0), (30.0, 110.0, 0.0)),
    ("dil-3.stl", (193.0, 20.0, 8.0), (33.5, 150.0, 0.0)),
]
SAMPLE_PARTS = [
    ("spoj-1.stl", (30.0, 20.0, 8.0), (90.0, 120.0, 0.0)),
    ("spoj-2.stl", (30.0, 20.0, 8.0), (140.0, 120.0, 0.0)),
]
...
def main():
    ...
    parser.add_argument('--height-mm', type=float, default=8.0, help='Nominal CAD height for geometry validation; does not scale STL')
    parser.add_argument('--sample', action='store_true', help='Separate two-piece joint test plate')
    args = parser.parse_args()
    require(args.height_mm > 0, 'Expected CAD height must be positive')
    parts = [(name, (size[0], size[1], args.height_mm), translation) for name, size, translation in (SAMPLE_PARTS if args.sample else PARTS)]
    stl_dir = HERE.parent / 'vzorek' if args.sample else STL_DIR
    output = HERE / 'TEST-spoj-pracovni-geometrie.3mf' if args.sample else OUTPUT
    ...
    ET.SubElement(model, "metadata", {"name": "Title"}).text = f"{'TEST spoje' if args.sample else 'Přepážka B'} — pracovní geometrie, {len(parts)} díly"
    ...
    result = {
        ...
        "limitations": ["Slicer import and toolpaths have not been checked.", "Filament type, hardness and print process are unknown.",
