...
def main():
    ...
    if options.sample:
        ...
        GCODE = OUT / 'plate_1.gcode'
        PROJECT = HERE / 'TEST-EXPERIMENT-spoj-Alzament-TPU95A.3mf'
    ...
    with zipfile.ZipFile(PROJECT) as archive:
        ...
        for (vertices,triangles), source_part in zip(meshes,geometry_audit['parts']):
            ...
            assert triangle_signature(vertices,triangles)==triangle_signature(positioned,original_triangles), 'Sliced project mesh or plate placement changed'
    ...
    result = {
        ...
        'all_project_triangles_match_stl_and_placement_with_0_00001mm_rounding': True,
    ...
    for axis, number in zip(axes, selected):
        ...
        axis.set(xlim=(80, 180) if options.sample else (15, 245), ylim=(110, 150) if options.sample else (55, 185), aspect='equal', xlabel='X [mm]', ylabel='Y [mm]')
    ...
    fig.suptitle(f"Skutečné extruzní dráhy · {'TEST spoje' if options.sample else 'přepážka B'} · {expected_objects} díly", fontsize=18, y=0.91)
