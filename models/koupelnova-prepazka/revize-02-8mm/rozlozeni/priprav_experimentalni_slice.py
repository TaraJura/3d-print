...
process_name = 'EXPERIMENT prepazka B TPU95A 0.20mm Kobra X 0.4'
filament_name = 'EXPERIMENT Alzament TPU95A Gray Kobra X 0.4 225C bed60C'
process.update({
    ...
    'filename_format': 'EXPERIMENT-prepazka-B-TPU95A-plate{plate_number}.gcode',
})
filament.update({
    'from': 'user', 'is_custom_defined': '1', 'name': filament_name, 'setting_id': filament_name,
    'filament_settings_id': [filament_name], 'filament_id': 'EXPERIMENT-Alzament-TPU95A-Gray',
...
with tempfile.TemporaryDirectory(prefix='prepazka-slicer-') as temporary:
    command = [
        ...
        '--metadata-value', f"{'TEST ' if args.sample else ''}EXPERIMENT prepazka B TPU95A;Local sliced experiment with geometry, profiles and G-code. Physical settings and sealing unverified. Bed temperature source conflict remains unresolved.",
        '--outputdir', str(OUT),
        '--export-3mf', '../TEST-EXPERIMENT-spoj-Alzament-TPU95A.3mf' if args.sample else '../EXPERIMENT-prepazka-B-Alzament-TPU95A.3mf',
        '--export-settings', str(OUT / 'effective-settings.json'),
        str(HERE / ('TEST-spoj-pracovni-geometrie.3mf' if args.sample else 'prepazka-B-pracovni-geometrie.3mf')),
    ]
